...
class GroupManager:
    ...
    def create_group(
        ...
        try:
            ...
            parent_group = self._resolve_parent_group(
    ...
    def update_group(
        ...
        try:
            ...
            if new_name is not None:
                ...
                if self._group_exists_in_parent(
                    new_name, group.parent, exclude_group=group
                ):
                    raise ValidationError(
                        f"Group '{new_name}' already exists in parent group"
                    )
    ...
    def delete_group(
        ...
        try:
            ...
            entries_count = len(
                self.keepass_handler.get_entries_in_group(
                    group, include_subgroups=False
                )
            )
            ...
            if move_entries_to and entries_count > 0:
                ...
                entries = self.keepass_handler.get_entries_in_group(
                    group, include_subgroups=False
                )
            ...
            # Log the operation
            self.logger.info(
                f"Group deleted: {group_name_for_log} (moved {moved_entries} entries)"
            )
    ...
    def move_group(
        ...
        try:
            ...
            group = self._resolve_group(group_id, group_name)
            target_parent = self._resolve_parent_group(
                target_parent_id, target_parent_name
            )
            ...
            if self._group_exists_in_parent(
                group.name, target_parent, exclude_group=group
            ):
                raise ValidationError(
                    f"Group '{group.name}' already exists in target parent"
                )
            ...
            # Log the operation
            self.logger.info(
                f"Group moved: {group.name} from {old_parent_name} to {new_parent_name}"
            )
    ...
    def list_groups(
        ...
        try:
            ...
            # Format groups
            formatted_groups = [
                self._format_group_response(
                    group, include_statistics=include_statistics
                )
                for group in groups
            ]
    ...
    def get_group_hierarchy(
        self, group_id: str = None, group_name: str = None, max_depth: int = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for entry in entries:
                if entry.password:
                    ...
                    if (
                        len(entry.password) >= 12
                        and any(c.isupper() for c in entry.password)
                        and any(c.islower() for c in entry.password)
                        and any(c.isdigit() for c in entry.password)
                    ):
                        strong_passwords += 1
                    else:
                        weak_passwords += 1
            ...
            entries_created_last_30_days = sum(
                1 for entry in entries if entry.ctime and (now - entry.ctime).days <= 30
            )
            entries_modified_last_30_days = sum(
                1 for entry in entries if entry.mtime and (now - entry.mtime).days <= 30
            )
    ...
    def _resolve_parent_group(
        self, parent_group_id: str = None, parent_group_name: str = None
    ):
    ...
    def _group_exists_in_parent(
        self, name: str, parent_group, exclude_group=None
    ) -> bool:
        """Check if a group with the given name exists in the parent group."""
        try:
            subgroups = self.keepass_handler.get_subgroups(
                parent_group, recursive=False
            )
    ...
    def _format_group_response(
        ...
        response = {
            ...
            "parent_id": (
                str(group.parent.uuid)
                if hasattr(group, "parent") and group.parent
                else None
            ),
            "parent_name": (
                group.parent.name if hasattr(group, "parent") and group.parent else None
            ),
            "created": (
                group.ctime.isoformat()
                if hasattr(group, "ctime") and group.ctime
                else None
            ),
            "modified": (
                group.mtime.isoformat()
                if hasattr(group, "mtime") and group.mtime
                else None
            ),
            "expires": (
                group.expires.isoformat()
                if hasattr(group, "expires") and group.expires
                else None
            ),
        ...
        if include_statistics:
            try:
                stats = self.get_group_statistics(
                    str(group.uuid) if hasattr(group, "uuid") else None, group.name
                )
    ...
    def _build_group_hierarchy(
        self, group, max_depth: int = None, current_depth: int = 0
    ) -> Dict[str, Any]:
    ...
    def _sort_groups(
        self, groups: List[Dict[str, Any]], sort_by: str
    ) -> List[Dict[str, Any]]:
