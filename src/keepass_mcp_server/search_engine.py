...
class SearchEngine:
    ...
    def search_by_url(
        ...
        try:
            ...
            for entry in entries:
                ...
                relevance = self._calculate_url_relevance(entry_url, url, domain, fuzzy_match)
                if relevance > 0:
                    entry_copy = entry.copy()
    ...
    def search_duplicates(
        ...
        try:
            ...
            duplicate_groups = [
                group for group in duplicates.values() 
                if len(group) > 1
            ]
            
            self.logger.info(f"Duplicate search: {len(duplicate_groups)} duplicate groups found")
    ...
    def _calculate_entry_similarity(
        ...
        if entry_tags and ref_tags:
            ...
            if common_tags:
                similarity += 0.1 * len(common_tags) / max(len(entry_tags), len(ref_tags))
    ...
    def _sort_results(
        ...
        if sort_by == "relevance":
            ...
            return sorted(results, key=lambda x: x.get('date_created', datetime.min), reverse=True)
        elif sort_by == "date_modified":
            return sorted(results, key=lambda x: x.get('date_modified', datetime.min), reverse=True)
        else:
            return results
