...
async def get_credential(
    entry_id: str, include_password: bool = True, include_history: bool = False
) -> str:
    ...
    try:
        ...
        entry = entry_manager.get_entry(
            entry_id, include_password=include_password, include_history=include_history
        )
...
async def search_weak_passwords(
    min_length: int = 8, require_complexity: bool = True
) -> str:
    ...
    try:
        ...
        weak_entries = search_engine.search_weak_passwords(
            all_entries, min_length=min_length, require_complexity=require_complexity
        )
...
def main():
    ...
    if not FASTMCP_AVAILABLE:
        print(
            "Fatal error: FastMCP library is required but not installed",
            file=sys.stderr,
        )
        sys.exit(1)
