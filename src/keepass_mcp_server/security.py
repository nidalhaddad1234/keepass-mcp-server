...
class PasswordManager:
    ...
    def __init__(self, service_name: str = "keepass-mcp-server"):
        ...
        if not KEYRING_AVAILABLE:
            self.logger.warning("Keyring library not available, passwords will not be cached")
    ...
    def get_password(self, username: str) -> Optional[str]:
        ...
        try:
            ...
            if password:
                self.logger.info(f"Password retrieved from keychain for user: {username}")
            return password
        except Exception as e:
...
class AuditLogger:
    ...
    def log_database_access(self, operation: str, user_id: str, details: str = ""):
        ...
        self.logger.info(f"DB_ACCESS - User: {user_id} - Operation: {operation} - Details: {details}")
    
    def log_entry_operation(self, operation: str, user_id: str, entry_title: str):
        """Log entry operations (without sensitive data)."""
        self.logger.info(f"ENTRY_{operation.upper()} - User: {user_id} - Entry: {entry_title}")
    
    def log_group_operation(self, operation: str, user_id: str, group_name: str):
        """Log group operations."""
        self.logger.info(f"GROUP_{operation.upper()} - User: {user_id} - Group: {group_name}")
    
    def log_security_event(self, event: str, user_id: str, details: str = ""):
        """Log security events."""
        self.logger.warning(f"SECURITY_EVENT - User: {user_id} - Event: {event} - Details: {details}")
    ...
    def log_session_event(self, event: str, user_id: str, session_token: str = ""):
        ...
        self.logger.info(f"SESSION_{event.upper()} - User: {user_id} - Token: {token_snippet}")
...
class SecurityManager:
    ...
    def authenticate_user(self, user_id: str, password: str, method: str = "password") -> str:
