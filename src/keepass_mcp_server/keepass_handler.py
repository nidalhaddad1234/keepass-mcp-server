...
class KeePassHandler:
    ...
    def __init__(self, config, security_manager: SecurityManager):
        ...
        self.key_file_path = Path(config.keepass_key_file) if config.keepass_key_file else None
    ...
    def unlock_database(self, password: str, key_file: str = None) -> Dict[str, Any]:
        ...
        with self.lock:
            try:
                ...
                key_file_to_use = key_file or (str(self.key_file_path) if self.key_file_path else None)
                ...
                if self.config.use_keychain:
                    self.security_manager.password_manager.store_password("database", password)
                ...
                unlock_info = {
                    ...
                    'database_version': str(self.database.version) if hasattr(self.database, 'version') else 'unknown'
                }
        ...
        with self.lock:
            ...
            try:
                ...
                info = {
                    ...
                    'last_saved': datetime.fromtimestamp(self.last_save_time).isoformat() if self.last_save_time else None,
                    'version': str(self.database.version) if hasattr(self.database, 'version') else 'unknown',
                    'encryption': 'AES-256' if hasattr(self.database, 'encryption_algorithm') else 'unknown',
                    ...
                    'has_recycle_bin': hasattr(self.database, 'recyclebin_group') and self.database.recyclebin_group is not None,
